...
-(void)refreshToken{
    ...
    if(  [ DB getStringById:@"token" fromTable:tabName]){
        ...
        token= [DB getStringById:@"token" fromTable:tabName];
    }
    
    if(  [ DB getStringById:@"refreshtoken" fromTable:tabName]){
        
        refreshtoken= [DB getStringById:@"refreshtoken" fromTable:tabName];
    }
    NSDictionary *dic = @{
                          @"client_id"    :@"ios",
                          @"client_secret":@"789",
