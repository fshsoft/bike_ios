...
- (void)setOverlayPickerView
{
    ...
    //扫描框图片的拉伸，拉伸中间一块区域
    UIImage *scanImage = [UIImage imageNamed:@"SM_jiao"];
...
- (void)layoutSubviews{
    ...
    //扫描框的宽
    CGFloat scanViewW = SCREEN_WIDTH-120;
    ...
    //最上部view
    _upView.frame = CGRectMake((SCREEN_WIDTH - scanViewW) * 0.5, 0, scanViewW,120);
    //底部view
    _downView.frame = CGRectMake((SCREEN_WIDTH - scanViewW) * 0.5, CGRectGetMaxY(_upView.frame) + scanViewW, scanViewW, SCREENH_HEIGHT - (CGRectGetMaxY(_upView.frame) + scanViewW));
    //扫码框
...
- (void)addAnimation{
    ...
    line.hidden = NO;
    CABasicAnimation *animation = [XYScanView moveYTime:2 fromY:[NSNumber numberWithFloat:0] toY:[NSNumber numberWithFloat:SCREEN_WIDTH-120 ] rep:OPEN_MAX];
