...
-(id)initWithFrame:(CGRect)frame time:(NSString *)time{
    self = [super initWithFrame:frame];
    if (self) {
        self.backgroundColor = [UIColor clearColor];
        fullRect = frame;
...
- (void)drawLabel:(CGContextRef)context {
    ...
    NSMutableAttributedString *attriButedText = [self formatBatteryLevel:self.num];
    ...
    NSMutableAttributedString *attriLabelText = [self formatLabel:self.time];
 ...
 *  @return 电量百分比文字参数
 */
-(NSMutableAttributedString *) formatBatteryLevel:(NSString*)text
{
